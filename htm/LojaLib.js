...
var sF$=(function(){
  ...
  function fnShowPriceFastPreview(Price,OriginalPrice,Cod,iMaxParcels,ProductID){
    iPL++;
    //console.log(ProductID+ " iPL="+ iPL +" Price="+Price +" OriginalPrice="+ OriginalPrice +" Cod="+ Cod);
    var idPrice=fnGetID("idProdPrice"+ProductID);
    var sPrice="";
  ...
  //Espiadinha animada
  var sFormItems="",IDProdEspiadinha=0;
  function getTagNameAdicional(IDProd){
    var getTags = function(IDProd){
      var aListNode=document.querySelectorAll('[name="FormItems'+IDProd+'"]');
      sFormItems=aListNode[0].innerHTML;
      IDProdEspiadinha=IDProd;
      aListNode[0].innerHTML="";
      $(document).on('click','.cd-close', function(event){sF$.restoreFormItems();});
      $('#cd-quick-view-coverlay').click(function(event){sF$.restoreFormItems();});
    };
    setTimeout(function(){getTags(IDProd);},100);
  }
  function restoreFormItems(){
    if(IDProdEspiadinha>0 && sFormItems!=""){
      var aListNode=document.querySelectorAll('[name="FormItems'+IDProdEspiadinha+'"]');
      aListNode[0].innerHTML=sFormItems;
      IDProdEspiadinha=0;
      sFormItems="";
  ...
  return{
    ...
    fnShowPriceFastPreview:fnShowPriceFastPreview,
    ...
    getTagNameAdicional:getTagNameAdicional,
    restoreFormItems:restoreFormItems
  }
